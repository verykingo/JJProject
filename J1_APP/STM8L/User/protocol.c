...
void protocol_repeat(void* ptr)
{
	...
	TIMER*	rpt_timer   = Pptr->rpt_timer;
		...
		case REP_WYAS_INC:	//通信继续，延时rep_cout*1秒重传
		{
			...
			{				
				...
				if(++rep_cout <= rep_nums)
				{
					...
					rpt_timer->cb_ticks		= SS_TO_TICKS(1)*rep_cout;
					TimerInsert(rpt_timer);
				}
			}
			else if(rpt_flag == 1)
			{
		...
		case REP_WYAS_EXP:	//通信继续，延时2^rep_cout*1秒重传
		{
			...
			{				
				...
				if(++rep_cout <= rep_nums)
				{
					...
					rpt_timer->cb_func	 	= protocol_repeat;
					rpt_timer->cb_data		= Pptr;
					rpt_timer->cb_ticks		= SS_TO_TICKS(1)*(2^rep_cout);
					TimerInsert(rpt_timer);
				}
			}
			else if(rpt_flag == 1)
		...
		case REP_WYAS_DELAY1:	//通信继续，延时1秒重传
		{
			...
			{				
				...
				if(++rep_cout <= rep_nums)
				{
					...
					rpt_timer->cb_func	 	= protocol_repeat;
					rpt_timer->cb_data		= Pptr;
					rpt_timer->cb_ticks		= SS_TO_TICKS(1);
					TimerInsert(rpt_timer);
				}
			}
			else if(rpt_flag == 1)
...
void protocol_command(void* ptr)
{
	...
	rep_timer->cb_ticks 	= MS_TO_TICKS(rep_time);
	TimerInsert(rep_timer);
